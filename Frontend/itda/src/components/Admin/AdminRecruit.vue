  ...
  <v-flex xs1>
      <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark class=" mt-7" v-on="on">New +</v-btn>
          </template>
          <v-card>
            <v-card-title>
              <span>공고 등록</span>
    ...
    <v-data-table
        ...
        <template v-slot:item.action="{ item }">
            <v-icon small="small" class="mr-2" @click="deleteItem(item)">
                edit
            </v-icon>
...
export default {
    ...
    methods: 
    {
        ...
        editItem (item) {
          this.editedIndex = this.allwanteds.indexOf(item)
          this.editedItem = Object.assign({}, item)
          this.dialog = true
        },

        deleteItem (item) {
          const index = this.allwanteds.indexOf(item)
          confirm('Are you sure you want to delete this Recruit?') && this.allwanteds.splice(index, 1)
        },
        ...
        save () {
          if (this.editedIndex > -1) {
            ...
            this.allwanteds.push(this.editedItem)
          }
          this.close()
        }
    },
    ...
    data () {
      return {
        ...
        headers: [
          ...
          { text: '마감일', 
            value: 'wanted.endDate' ,align: 'center'},
