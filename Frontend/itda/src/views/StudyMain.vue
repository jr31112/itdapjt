...
export default {
    ...
    methods:{
        getDefaultStudies(){
            axios.get('http://54.180.140.163:8197/itda/api/getAllStudy')
            .then(response=>{
        ...
        if (!select && this.formData.stype!=4){
            ...
            if (this.$refs.form.validate()) {
                axios.post('http://54.180.140.163:8197/itda/api/createStudy', this.formData, {'headers' : {"jwt-auth-token": localStorage.getItem("access_token")}})
                .then(()=>{
                    alert('스터디를 생성하였습니다.')
