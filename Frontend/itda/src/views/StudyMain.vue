...
export default {
    ...
    methods:{
        getStudies(){
            axios.get('https://192.168.31.54:8197/itda/api/getAllStudy')
            .then(response=>{
        ...
        if (!select){
            ...
            if (this.$refs.form.validate()) {
                this.snackbar = true
                axios.post('https://192.168.31.54:8197/itda/api/createStudy', this.formData, {'headers' : {"jwt-auth-token": localStorage.getItem("access_token")}})
                .then(()=>{
                    alert('스터디를 생성하였습니다.')
